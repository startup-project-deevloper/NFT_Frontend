...
const AssistanceNFTMediaTab = React.memo((props: any) => {
                ...
                <div
                  style={{
                    backgroundImage: user
                      ? `url(${user.imageUrl ? user.imageUrl : getRandomAvatar()})`
                      : "none",
  ...
  return (
    ...
    >
      ...
      {props.creation && (
        <>
          <div className={classes.bigTitle}>Pod's Token</div>
          {/* <Box
            color="#7F6FFF"
            fontSize="14px"
            display="flex"
            ...
            <Grid item xs={12} md={12} style={{ marginTop: "24px" }}>
              <div className={classes.inputContainer}>
                <Autocomplete
                            ...
                            <Avatar
                              ...
                              url={
                                typeof option !== "string" && option.imageUrl
                                  ? option.imageUrl
                                  : option.annonAvatar
                                    ? require(`assets/anonAvatars/${option.annonAvatar}`)
                                    : getRandomAvatar()
                              }
                              size="medium"
                            />
                            <Box ml="5px" color="#1A1B1C" fontSize="12px" fontFamily="Agrandir">
                              {option.firstName ?? (option.urlSlug ? `@${option.urlSlug}` : "")}
                            </Box>
                          </Box>
                          <Box
                            display="flex"
                  ...
                  getOptionLabel={option => option.firstName ?? (option.urlSlug ? `@${option.urlSlug}` : "")}
