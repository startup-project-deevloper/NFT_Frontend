...
export default function PriceGraph({ graphData, title, subTitle }) {
  const classes = PriceGraphStyles();
	...
	const [intervals, setIntervals] = useState<Array<string>>(['1H']);
	...
	useEffect(() => {
		if (graphData && graphData.length) {
			...
			if (graphData[1] && graphData[1].length) {
				intervalData.push('1D');
				mapData.push(['1D', graphData[1]]);
			}

			if (graphData[2] && graphData[2].length) {
				intervalData.push('7D');
				mapData.push(['7D', graphData[2]]);
			}
			// @ts-ignore
  	...
  	}, [seriesesData, width, height, intervals])
