...
import StakingPodCard from "components/PriviMusicDao/components/Cards/StakingPodCard";
import { MasonryGrid } from "shared/ui-kit/MasonryGrid/MasonryGrid";
import PodStakingModal from "components/PriviMusicDao/modals/PodStakingModal";
...
const Staking = ({ pod, handleRefresh }) => {
  ...
  return (
    <Box mt={3}>
      ...
      <MasonryGrid
        gutter={"21px"}
        data={mockStakings.filter((itm, i) => i < 3)}
        renderItem={(item, index) => <StakingPodCard key={`active-${index}`} staking={item} />}
      ...
      <MasonryGrid
        gutter={"21px"}
        data={mockStakings.filter((itm, i) => i < 3)}
        renderItem={(item, index) => <StakingPodCard key={`expired-${index}`} staking={item} expired />}
        columnsCountBreakPoints={{ 300: 1, 765: 2, 900: 3 }}
      />
