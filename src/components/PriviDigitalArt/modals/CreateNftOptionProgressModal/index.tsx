...
import { useCreateNftOptionProgressModalModalStyles } from "./index.styles";

require("dotenv").config();
const isDev = process.env.REACT_APP_ENV === "dev";

export default function CreateNftOptionProgressModalModal({
  open,
  onClose,
  txSuccess,
  hash,
  ...
  const classes = useCreateNftOptionProgressModalModalStyles();
  ...
  return (
    <Modal showCloseIcon isOpen={open} onClose={onClose} className={classes.root} size="small">
      ...
      <Box className={classes.header1} mt={1} mb={2}>
        {txSuccess === true
          ? "Your NFT has been locked successfully. You can see it by clicking Manage Portfolio now."
          : txSuccess === false
          ? ""
