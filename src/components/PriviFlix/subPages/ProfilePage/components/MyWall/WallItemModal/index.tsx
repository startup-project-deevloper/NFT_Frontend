    ...
    <Box display="flex" alignItems="center" className={styles.responseContainer}>
      <div className={styles.userCardImageWallPost}>
        <div
          ...
          onClick={() => {
            history.push(`/flix/profile/${response.userId}`);
        ...
        <Box ml="8px" className={styles.commentUserInfo}>
          ...
          <Box fontSize="12px" fontWeight={800} color="#ff5954" className={styles.userUrlSlug}>
            @{response?.urlSlug}
          </Box>
        </Box>
      </div>
  ...
  return (
    <>
      ...
      <Box display="flex" alignItems="flex-end" justifyContent="space-between" mb={"48px"}>
        <Box className={styles.userInfo} onClick={() => history.push(`/flix/profile/${item.createdBy}`)}>
          <Avatar
            url={
              item?.userImageURL ??
        ...
        <>
          ...
          {!onlyDisplay ? (
            <div className={styles.inputResponseWallPost}>
              ...
              />
              <img src={require("assets/icons/send_white.png")} alt={"send"} onClick={makeResponse} />
            </div>
          ) : (
            <></>
          )}
