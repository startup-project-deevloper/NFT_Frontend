          ...
          medias.forEach((m, index) => {
            ...
            if (!m.price) {
              if (
                ...
                medias[index].price = `${m.FundingToken.toUpperCase() ?? ""} ${medias[index].PricePerSecond && m.PricePerSecond !== 0 ? m.PricePerSecond + "/per sec." : ""
                  }`;
  ...
  return (
    <div className={classes.mainContent} ref={scrollRef} id="profile-infite-scroll">
      ...
      >
        ...
        <Box display="flex" alignItems="center" gridColumnGap={2}>
          {ownUser && (
            <PrimaryButton
              size="medium"
      ...
      <Box mb={4}>
        <LoadingWrapper loading={priviUser === undefined}>
          {activeTab === 0 && ownUser ? (
