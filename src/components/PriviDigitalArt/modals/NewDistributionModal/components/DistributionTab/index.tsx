...
const DistributionTab = (props: any) => {
  ...
  const maxInvestorsShare = React.useMemo(() => {
    ...
    const creatorsSum = props.pod.Collabs.map((e: any) => Number(e.sharingPercent ?? 0)).reduce(
  ...
  const getMaxCreatorShare = React.useCallback(
    (ownIndex: number) => {
      if (!props.pod || !props.pod.Collabs) return 100;
      const creatorsSum = props.pod.Collabs.filter((e: any, index: number) => index !== ownIndex)
    ...
    <div className={classes.generalNftMediaTab}>
      ...
      {props.pod.Collabs?.map((creator, index) => {
        return (
          ...
          >
            <Box display="flex">
              <Avatar size={34} image={creator.imageUrl ?? getDefaultAvatar()} radius={25} bordered rounded />
      ...
      <Box className={classes.distribBox} display="flex" alignItems="center" justifyContent="space-between">
        ...
        <InputWithLabelAndTooltip
          ...
          inputValue={props.pod.InvestorShare || 0}
        />
      </Box>
    </div>
  );
