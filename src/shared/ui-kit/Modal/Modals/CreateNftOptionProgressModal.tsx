...
import { useCreateNftOptionProgressModalStyles } from "./CreateNftOptionProgressModal.styles";
import { CopyToClipboard } from "react-copy-to-clipboard";

require("dotenv").config();
const isDev = process.env.REACT_APP_ENV === "dev";

export default function CreateNftOptionProgressModal({
  open,
  onClose,
  reserveInProgress,
  reserveSuccess,
  ...
  reserveSuccess: boolean | null;
  hash?: string;
  network: string;
}) {
  const classes = useCreateNftOptionProgressModalStyles();
  ...
  return (
    <Modal showCloseIcon isOpen={open} onClose={onClose} className={classes.root} size="medium">
      ...
      <div className={classes.title}>
        {reserveInProgress
          ? "Creating Reserve Offer"
          : reserveSuccess === false
          ? "Your NFT Option is failed"
          : reserveSuccess === true
          ? "Your NFT Option is Created"
          : null}
      </div>

      <Box>
        {reserveInProgress
          ? `reserve is proceeding on ${network.replace(" Chain", "").toUpperCase()} Mainnet.
                This can take a moment, please be patient and do not close this window`
          : reserveSuccess === false
          ? "Unfortunately there was an error during the reserve. Please try again later."
          : reserveSuccess === true
          ? ``
          : null}
      </Box>
