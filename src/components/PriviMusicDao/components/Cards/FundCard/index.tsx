...
import ProposalDetailModal from "components/PriviMusicDao/modals/ProposalDetailModal";
...
export const FundCard = props => {
  ...
  const handleAccept = async () => {
    ...
    const voteContractResponse = await web3APIHandler.PodManager.voteForPodProposal(web3, account!, {
      id: parseInt(proposal.Id),
    });

    if (!voteContractResponse) {
    ...
    const executeContractResponse = await web3APIHandler.PodManager.executePodProposal(web3, account!, {
      id: parseInt(proposal.Id),
    });
