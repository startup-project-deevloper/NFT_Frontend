...
const PriviSocialConnect = () => {
        ...
        .then(async res => {
          if (res.data.success === true) {
            ...
            if (data.status === "authorized") {
              ...
              API.signInWithMetamaskWallet(account, web3, "Privi Social").then(res => {
                if (res.isSignedIn) {
                  setSignedin(true);
  ...
  useEffect(() => {
    ...
    const intervalRef = setInterval(() => {
      var doc = document.getElementById("earlyAccessSocial");
  ...
  return (
    <Box className={classes.container}>
      <Box className={classes.navigationContainer}>
        <img src={require("assets/logos/privi_social.png")} className={classes.logo} />
        <Box display="flex" alignItems="center" justifyContent="center">
          <img width="70%" src={require("assets/socialImages/social_title_green.svg")} alt="Social" />
        </Box>
        <Box
        ...
        <Box className={classes.mainContainer} display="flex" flexDirection="column">
          <Box className={classes.titleContainer} display="flex" alignItems="center" justifyContent="center">
            <Box position="relative">
              <img
                src={require("assets/logos/privi_social.png")}
                className={classes.titleLogo}
                alt="Privi Social"
              />
              <span id="earlyAccessSocial" className={classes.earlyAccess}>
                early access
              </span>
            </Box>
          </Box>
              ...
              {waitlisted ? (
                ...
                <>
                  ...
                  <Box className={classes.twitterShareContainer}>
                    <span>
                      I just joined the @priviprotocol waitlist for <b>Privi Social</b> with {account}! Join
                      me to be one of the first people on <b>Privi Social</b> and a chance to win up to 200
                      USDC!{" "}
                      <a href="https://bit.ly/2TLTvXD" target="_blank">
                        https://bit.ly/2TLTvXD
                      </a>
                    </span>
                    <LoadingWrapper loading={tweetWaitlistLoading}>
                      <TwitterShareButton
                        title={`I just joined the @priviprotocol waitlist for Privi Social with ${account}! Join me to be one of the first people on Privi Social and a chance to win up to 200 USDC!`}
                        url="https://bit.ly/2TLTvXD"
                        ref={twitterButton}
                      >
                        <button
...
export default PriviSocialConnect;
