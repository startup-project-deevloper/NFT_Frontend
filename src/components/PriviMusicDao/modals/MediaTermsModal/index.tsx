...
const mediaCopyTermsModal = ({ open, handleClose, pod, media, handleRefresh }) => {
  ...
  const handleOk = () => {
    ...
    if (payload) {
      ...
      musicDAORegisterMedia(payload, additionalData).then(resp => {
        if (resp?.success) {
    ...
    <>
      <Modal size="medium" isOpen={open} onClose={handleClose} showCloseIcon>
        ...
        <Box className={classes.footer} mt={3}>
          ...
          {tabmediaCopyTerms == 2 && (
            <Box className={classes.footerRight}>
              <PrimaryButton size="medium" onClick={() => handleOpenSignatureModal()} isRounded>
                Register Conditions
              </PrimaryButton>
            </Box>
          )}
