...
export function DiscussionPage(props) {
  ...
  const sendMessage = (audioMessage?: string) => {
    ...
    if (socket) socket.emit("add-message-podDiscussion", messageObj);
      ...
      <Box className="discussion-posts">
        ...
        <Box className="selected-discussion">
          {Object.keys(topicView).length > 0 ? (
            ...
            >
              <LoadingWrapper loading={messagesLoading} theme={"blue"} />
              {messages.map((val, index) => (
                <Box
                  ...
                  id={val.created}
                >
                  <Box className={val.from === user.id ? "avatar" : "otherAvatar"}>
                    <Avatar
                      size="small"
                      url={
                        users.find(u => u.id === val.from)?.imageUrl
                          ? users.find(u => u.id === val.from)?.imageUrl
                          : users.find(u => u.id === val.from)?.anonAvatar
                          ? require(`assets/anonAvatars/${users.find(u => u.id === val.from)?.anonAvatar}`)
                          : getRandomAvatar()
                      }
                    />
                  </Box>
              ...
              {!audioMessage && (
                <form onSubmit={handleSubmit}>
                  <TextareaAutosize
                    ...
                    className="comment"
                  />
                  <PrimaryButton size="small" disabled={disableSubmit} style={{ background: "#431AB7" }}>
