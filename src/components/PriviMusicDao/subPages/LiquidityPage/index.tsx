...
import { FontSize, Color, StyledDivider, SecondaryButton, Variant } from "shared/ui-kit";
...
const FreeHoursChartConfig = {
  ...
  configurer: (config: any, ref: CanvasRenderingContext2D): object => {
    let gradient = ref.createLinearGradient(0, 0, 0, ref.canvas.clientHeight * 2);
      ...
      <Box className={classes.content}>
        <Box display="flex" flexDirection="column" alignItems="center">
          ...
          <h3 className={classes.headerSubTitle}>
            Get rewards, for providing liquidity stablecoins on Privi.
          </h3>
          <Grid container direction={"row"} spacing={3} wrap={"wrap"} className={classes.descriptionGroup}>
            <Grid item xs={12} sm={6}>
              <Box display="flex" flexDirection="row">
                <img
                  src={require("assets/musicDAOImages/music-green.png")}
                  alt="music"
                  style={{ marginTop: "-35px" }}
                  className={classes.descriptionLogo}
                />
                <Text className={classes.description} bold>
                  Provide liquidity and earn rewards from hour of music not consumed by users
                </Text>
              </Box>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Box display="flex" flexDirection="row">
                <img
                  src={require("assets/musicDAOImages/yield-green.png")}
                  alt="music"
            ...
            <Grid item xs={12} sm={6}>
              <Box display="flex" flexDirection="row">
                <img
                  src={require("assets/musicDAOImages/distribution.png")}
                  alt="music"
                  style={{ width: 70, marginLeft: 20, marginRight: 20, minWidth: 70, height: 90 }}
                />
                <Text className={classes.description} bold>
                  Access immediate funds at a collateralisation ratio to generate further yield
                  <br />
                  <Text className={classes.subDescription} size={FontSize.S} color={Color.White} bold>
            ...
            <Box display="flex" alignItems="center" gridColumnGap={18}>
              {pools.map((pool, index) => (
                <div key={`pool-card-${index}`} className={classes.poolItem}>
                  <PoolCard
                    data={pool}
                    handleMore={() => history.push(`/trax/liquidity/pool_detail/0x1234567`)}
                  />
                </div>
              ))}
            </Box>
        ...
        <Box display="flex" flexDirection="column" mt={4} mb={4}>
          ...
          <Box className={classes.transactionOption} display="flex" flexDirection="row">
            <Box
              display="flex"
              flexDirection="row"
              alignItems="center"
