    ...
    <Modal size="medium" isOpen={open} onClose={handleClose} showCloseIcon>
                  ...
                  <Box display="flex" alignItems="center" mt={1}>
                    {hashTags ? (
                      ...
                      hashTags.map((hashtag, i) => {
                        if (i === 0) {
                          return <HashtagLabel key={i} value={hashtag} index={i} main={true} remove={handleRemoveTag} />;
                        } else {
                          return <HashtagLabel key={i} value={hashtag} index={i} main={false} remove={handleRemoveTag} />;
                        }
                      })
                    ) : (
                      <></>
      ...
      <Box display="flex" alignItems="center" justifyContent="flex-end" width="100%" mt={6}>
        ...
        <LoadingWrapper loading={creationProgress} theme="green">
          <SocialPrimaryButton onClick={createPost} style={{ background: "#431AB7", borderColor: "#431AB7" }}>Publish Post</SocialPrimaryButton>
        </LoadingWrapper>
      </Box>
...
const HashtagLabel = (p) => {
  return (
    <Box color="rgb(0, 255, 21)" mr={2} mb={2} position="relative">
      <Box display="flex" justifyContent="center">{p.value}</Box>
      <Box position="absolute" right={-10} top={-10} style={{ cursor: "pointer" }} onClick={() => p.remove(p.value)}>
        <svg width="8" height="8" viewBox="0 0 16 16" fill="none">
          <path d="M15 1L1 15M1.00001 1L15 15" stroke="#707582" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </Box>
    </Box>
  );
