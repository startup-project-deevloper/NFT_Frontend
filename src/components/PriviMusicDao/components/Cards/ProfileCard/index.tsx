...
import MediaDetailsModal from "components/PriviMusicDao/modals/MediaDetailsModal";
...
export default function ProfileCard({
  ...
  const [mediaViews, setMediaViews] = useState<any>(0);
  ...
  useEffect(() => {
    if (item) {
      setItemName(
        type === "Social"
          ? item.TokenName
          : item.name ?? item.MediaName ?? item.Name ?? item.Title ?? item.title ?? "Item Name"
      );
      ...
      setFruits(0);
      setShares(0);
      setMediaViews(item.TotalViews ?? 0);
      setPrice(
        `${item.FundingToken ?? "ETH"} ${
          item.InitialSupply ?? item.InitialPrice ?? item.Price ?? item.price ?? "N/A"
        }`
      );
      setUsdPrice(0);
        ...
        <div className={classes.bottomContent}>
          <Box display="flex" alignItems="center" justifyContent="space-between">
            <Box display="flex" alignItems="center" maxWidth="80%">
              ...
              <Box
                style={{ overflow: "hidden", textOverflow: "ellipsis", color: "#181818", fontWeight: 800 }}
              >
                {itemName}
              </Box>
            </Box>
            <Box className={classes.fruitContainer}>
