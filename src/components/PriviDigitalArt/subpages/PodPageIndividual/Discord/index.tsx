...
const Discord = React.memo((props: any) => {
  ...
  const getMessages = (isNew: boolean = true): Promise<number> => {
    return new Promise((resolve, reject) => {
      ...
      axios
        .get(`${ServerURL()}/podDiscussion/new/newChat/getMessages/${props.podId}/${props.chatType}/${props.chatId}/${isNew ? 0 : pageIndex}`)
        .then(async response => {
  ...
  const handleFilesAttachment = async files => {
    for (let i = 0; i < files.length; i++) {
      ...
      if(msg && msg.type !== 'text' && msg.url && msg.url.newFileCID) {
        let file = await onDownloadNonDecrypt(msg.url.newFileCID, fileCID => downloadWithNonDecryption(fileCID, false));
  ...
  return (
    <div className={`podDiscordFullPage ${props.theme && props.theme === "dark" ? "dark" : ""}`}>
      <Box display="flex"
           height="100%"
           width="100%"
           className="podDiscordGrid">
        <div className="podDiscordChatGrid">
          {!missingIds ? (
            <div className="podDiscordChat">
              <div className="podMessagesDiscordChat"
                   id="messageContainer"
                   ref={itemListRef}
                    ...
                    messages.map((item, i) => {
                      return (
                        <div key={i} id={item.id}>
                          <MessageDiscordChat message={item}
          ...
          <div className="noMessagesLabelChat"
