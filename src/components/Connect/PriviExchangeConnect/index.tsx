...
const PriviExchangeConnect = () => {
        ...
        .then(async res => {
          if (res.data.success === true) {
            ...
            if (data.status === "authorized") {
              ...
              API.signInWithMetamaskWallet(account, web3, "Privi Exchange").then(res => {
                if (res.isSignedIn) {
                  setSignedin(true);
                  ...
                  window.location.href = "https://ddex.privi.store";
                } else {
                  if (res.message) {
  ...
  useEffect(() => {
    ...
    const intervalRef = setInterval(() => {
      var doc = document.getElementById("earlyAccessExchange");
        ...
        <Box className={classes.mainContainer} display="flex" flexDirection="column">
          <Box className={classes.titleContainer} display="flex" alignItems="center" justifyContent="center">
            <Box position="relative">
              <img
                src={require("assets/logos/privi_exchange_large.png")}
                alt="Privi Exchange"
                className={classes.pixLogo}
              />
              <span id="earlyAccessExchange" className={classes.earlyAccess}>
                early access
              </span>
            </Box>
          </Box>
          {account ? (
              ...
              {waitlisted ? (
                ...
                <>
                  ...
                  <Box className={classes.twitterShareContainer}>
                    <span>
                      I just joined the @priviprotocol waitlist for <b>Privi Exchange</b> with {account}! Join
                      me to be one of the first people on <b>Privi Exchange</b> and a chance to win up to 200
                      USDC!{" "}
                      <a href="https://bit.ly/3jbpyZs" target="_blank">
                        https://bit.ly/3jbpyZs
                      </a>
                    </span>
                    <LoadingWrapper loading={tweetWaitlistLoading}>
                      <TwitterShareButton
                        title={`I just joined the @priviprotocol waitlist for Privi Exchange with ${account}! Join me to be one of the first people on Privi Exchange and a chance to win up to 200 USDC!`}
                        url="https://bit.ly/3jbpyZs"
                        ref={twitterButton}
                      >
                        <button
            ...
            <>
              <Box className={classes.titleDescription}>
                ...
                <span style={{ color: "#D9F66F", fontWeight: 300 }}>
...
export default PriviExchangeConnect;
